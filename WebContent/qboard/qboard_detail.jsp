...
<script>
	function delConfirm() {
		...
		if(con) {
			location.href = "qboard_deleteOk.jsp?qno=<%=vo.getQno() %>";
		}
	}
...
<body>
	<div id="body">
		<div id="header" class="bar">
            ...
            <div class="login_area">
             ...
             <div id="nav">
				<div id="menuwrapper">
					<a href="qboard_list.jsp">문의 게시판</a>
					<a href="#"></a>
					<a href="#"></a>
				</div>
			</div>
	...
	<div id="container">
		...
		<div> 
			...
			<div><%=vo.getQcontents() %></div>
		</div>
	</div>
	<a href="qboard_list.jsp"><input type="button" value="목록" /></a>
	<%if(mno == vo.getMno()){ %><input type="button" onclick="modifyQboard();" value="수정" /><%} %>
	<%if(mno == vo.getMno()){ %><input type="button" value="삭제" onclick="delConfirm()" /></a><%} %>
	
	<br />
	<br />
	
	<div>
		<form action="answerOk.jsp">
			<table>
				<%
					if(mno==0) {
				%>
				<tr>
					<th>댓글은 로그인 후 이용가능합니다.</th>
				</tr>
				<%
					} else {
				%>
	...
	<script>
		function modifyQboard(){
			location.href="qboard_modify.jsp?qno=<%=vo.getQno() %>";
		}
	</script>
